...
- (void)scheduleOneNotification {
  if ([notificationsQueue count]) {
    ...
    if ([[notif fireDate] compare:[NSDate dateWithTimeIntervalSinceNow:5.0]] == NSOrderedAscending) {
     // If the date is in the past, schedule it for one day from now
      NSDateComponents *components = [[[NSDateComponents alloc] init] autorelease];
      NSCalendar *gregorian = [[[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar] autorelease];
      [components setDay:1];
      NSLog(@"shifting %@ forward a day", [notif fireDate]);
      [notif setFireDate:[gregorian dateByAddingComponents:components toDate:[notif fireDate] options:0]];
    }
...
- (void)updateNotifications {
  ...
  BOOL weekends = [defaults boolForKey:@"days"];
  ...
  NSArray *themes = [NSArray arrayWithObjects:@"beep", @"roman", @"speech", @"chord", nil];
  ...
  if (enabled) {
    ...
    NSUInteger nextHour = [dateComponents hour] + 1;
    [dateComponents setHour:nextHour];
    ...
    //NSInteger weekendShift = [dateComponents weekday] - [gregorian firstWeekday];
    
    for (int i = 0; i < 24; i++) {
      NSUInteger anHour = (nextHour + i) % 24;
      ...
      if (isValid) {
        ...
        for (int j = 0; j < (frequency && halvesAreValid ? 4 : 1); j++) {
          ...
          NSUInteger hourName = (nextHour + i) % 12;
...
- (void)playTestSound {
  ...
  if ([notifications count]) {
    UILocalNotification *notif = [notifications lastObject];
